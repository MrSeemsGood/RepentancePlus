...
FallingKnifeHelper = Isaac.GetEntityVariantByName("Falling Knife Helper")
AnimatedItemDummyEntity = Isaac.GetEntityVariantByName("AnimatedItemDummyEntity")
...
CustomFamiliars = {
	...
	ULTRA_FLESH_KID_L1 = Isaac.GetEntityVariantByName("Ultra Flesh Kid (level 1)"),
	ULTRA_FLESH_KID_L2 = Isaac.GetEntityVariantByName("Ultra Flesh Kid (level 2)"),
	ULTRA_FLESH_KID_L3 = Isaac.GetEntityVariantByName("Ultra Flesh Kid (level 3)"),
	ULTRA_FLESH_KID_L4 = Isaac.GetEntityVariantByName("Ultra Flesh Kid (level 4)")
}
...
function rplus:OnGameStart(Continued)
	...
	for i = 0, game:GetNumPlayers() - 1 do
		...
		Isaac.GetPlayer(i):AddCacheFlags(CacheFlag.CACHE_ALL)
	...
	game:GetItemPool():RemoveTrinket(CustomTrinkets.TORN_PAGE)
	game:GetItemPool():RemoveTrinket(CustomTrinkets.EMPTY_PAGE)
					...
					if level:GetCurrentRoomDesc().ListIndex == CustomData.Trinkets.ANGELS_CROWN.treasureRoomsData[i].index then
						if player:HasTrinket(CustomTrinkets.ANGELS_CROWN) then
							...
							for _, collectible in pairs(Isaac.FindByType(5, 100, -1, false, false)) do
								...
								collectiblePickup.Price = player:GetTrinketMultiplier(CustomTrinkets.ANGELS_CROWN) > 1 and 7 or 15
							end
						else
...
function rplus:OnGameUpdate()
	...
	for i = 0, game:GetNumPlayers() - 1 do
		...
		-- might seems like a bit of an overkill but it's better this way
	...
	for _, sg in pairs(Isaac.FindByType(6, CustomPickups.SLOT_STARGAZER, -1, false, false)) do
		...
		if SGSprite:IsPlaying("Teleport") and SGSprite:IsEventTriggered("Disappear") then
			repeat
				newID = CustomItempools.STARGAZER[math.random(#CustomItempools.STARGAZER)]
			-- just check for the main player, who cares?
			until not Isaac.GetPlayer(0):HasCollectible(newID)
	...
	for _, enemy in pairs(Isaac.FindInRadius(Vector(320, 280), 400, EntityPartition.ENEMY)) do
		...
		if isCollidingWithAstralChain(enemy) and game:GetFrameCount() % 3 == 0 then
			enemy:TakeDamage(1, 0, EntityRef(enemy), 0)
		end
	end
	for _, chain in pairs(Isaac.FindByType(865, 10, 1, true, false)) do
...
function rplus:OnItemUse(ItemUsed, _, Player, UseFlags, Slot, _)
	...
	if ItemUsed == CustomCollectibles.AUCTION_GAVEL then
		...
		auctionCollectible.Price = Player:HasCollectible(CollectibleType.COLLECTIBLE_STEAM_SALE) and 6 or 15
...
function rplus:OnCardUse(CardUsed, Player, _)	
	...
	if not (room:GetType() == RoomType.ROOM_BOSS or room:GetType() == RoomType.ROOM_BOSSRUSH) then
...
function rplus:PostPlayerUpdate(Player)
		...
		if Player:GetData().PressFrame and game:GetFrameCount() <= Player:GetData().PressFrame + 6 then 
			...
			and Input.IsActionTriggered(Player:GetData().ButtonPressed, Player.ControllerIndex) then
				...
				(not CustomData.Items.MAGIC_PEN.CreepSpewCooldown or CustomData.Items.MAGIC_PEN.CreepSpewCooldown <= 0) then
					...
					for i = 1, 15 do
						Isaac.Spawn(1000, EffectVariant.PLAYER_CREEP_HOLYWATER_TRAIL, 4, Player.Position + creepDirection * i, Vector.Zero, Player)
					end
	...
	and Player:GetActiveCharge(0) < 6 then
		if not Player:GetData()['usedLem'] then
			local heartcharges = 6 - Player:GetActiveCharge(0)
...
function rplus:OnGameRender()
	...
	for i = 0, game:GetNumPlayers() - 1 do
		...
		if player:HasTrinket(CustomTrinkets.GREEDS_HEART) and not isInGhostForm(player) then
			...
			-- did anybody get this reference? it's from Stuff Made Here, cool channel btw!
		...
		if CustomData and CustomData.Items.ANGELS_WINGS.AttackCooldown then
	...
	if type(NPC:GetData().IsCrippled) == 'boolean' and NPC:GetData().IsCrippled == true then
		...
		if (NPC:HasMortalDamage() or NPC.Friction <= 0.2) and not NPC:GetData().CrippleDeathBurst then
			...
			for i = 0, game:GetNumPlayers() - 1 do
				...
				if player:HasCollectible(CustomCollectibles.BOOK_OF_LEVIATHAN) then
					RNGobj:SetSeed(Random() + 1, 1)
					local roll = RNGobj:RandomFloat() * 100
		...
		for i = 0, game:GetNumPlayers() - 1 do
			...
			if Pickup.Variant == 10 and not noCustomHearts and room:GetType() ~= RoomType.ROOM_BOSS then
				...
				if st == HeartSubType.HEART_FULL or st == HeartSubType.HEART_HALF then
					...
					baseChance = 1.25 baseChanceChanged = true end
				...
				elseif st == HeartSubType.HEART_SOUL then
					...
					-- 12.5% fettered heart
					if roll < baseChance and stage > 1 then Pickup:Morph(5, 10, CustomPickups.TaintedHearts.HEART_FETTERED, true, true, false)
					-- 2.5% zealot heart
...
function rplus:PickupCollision(Pickup, Collider, _)	
	...
	and Pickup.Variant == 10 then
		...
		if player:GetActiveItem(1) == CustomCollectibles.STARGAZERS_HAT and player:GetActiveItem(0) ~= CustomCollectibles.STARGAZERS_HAT then HatSlot = 1 end
	...
	if Pickup.Variant == CustomPickups.FLESH_CHEST and Pickup.SubType == 0 then
		if player:GetSprite():IsPlaying("Hit") or sp:IsPlaying("Appear") or sp:IsPlaying("TakeHealth") then return false end
		
		player:TakeDamage(1, DamageFlag.DAMAGE_RED_HEARTS | DamageFlag.DAMAGE_NO_PENALTIES, EntityRef(Pickup), 16)
		...
		if ((CustomData.FleshChestConsumedHP > 2 and math.random(100) < FLESHCHEST_OPEN_CHANCE) or CustomData.FleshChestConsumedHP == 6) then
			...
			if DieRoll < 30 then
				...
				repeat
					newTrinket = CustomItempools.FLESHCHEST_trinkets[math.random(#CustomItempools.FLESHCHEST_trinkets)]
				until not player:HasTrinket(newTrinket)
				Isaac.Spawn(5, 350, newTrinket, Pickup.Position, Vector.FromAngle(math.random(360)) * 4, Pickup)
			end
		end
	end
		...
		if type(Pickup:GetData().Data) == 'nil' then
			...
			if pf % 6 == 0 then
				...
				elseif Pickup:GetData().Data == "sale price" then
					Pickup.Price = math.min(math.random(6 + pf // 30, 12 + pf // 30), 99)
				end
			end
		end
	end
...
function rplus:OnTearUpdate(Tear)
	...
	if Tear.FrameCount == 1 then
		if Player:HasCollectible(CustomCollectibles.CEREMONIAL_BLADE) and math.random(100) <= CEREM_DAGGER_LAUNCH_CHANCE then
			...
			s:Load("gfx/002.120_ceremonial_blade_tear.anm2", true)
			s:Play("Idle")
		end
		
		if Tear.Variant ~= CustomTearVariants.CEREMONIAL_BLADE 
		and Tear.Variant ~= CustomTearVariants.VALENTINES_CARD 
		and Tear.Variant ~= CustomTearVariants.ANTIMATERIAL_CARD then
			...
			if Player:HasCollectible(CustomCollectibles.ANGELS_WINGS) then
				tearSprite:Load("gfx/002.125_dogma_feather_tear.anm2", true)
				tearSprite:Play("MoveVert")
			end
		end
...
function rplus:TearCollision(Tear, Collider, _)
	...
	if Tear.Variant == CustomTearVariants.VALENTINES_CARD and Collider.Type ~= 951 then
		Collider:AddEntityFlags(EntityFlag.FLAG_FRIENDLY | EntityFlag.FLAG_CHARM | EntityFlag.FLAG_PERSISTENT)
	end
...
function rplus:OnCacheEvaluate(Player, Flag) 
	...
	if Flag == CacheFlag.CACHE_DAMAGE then
		...
		if CustomData.Cards.SACRIFICIAL_BLOOD.Data then
			if Player:GetData()['usedBlood'] then
				Player.Damage = Player.Damage + CustomStatups.Damage.SACRIFICIAL_BLOOD * (CustomData.Cards.SACRIFICIAL_BLOOD.NumUses - Step / 50)
			end
		end
...
function rplus:FamiliarUpdate(Familiar)	
	...
	if Familiar.FrameCount == 5
	and Familiar.Variant == FamiliarVariant.WISP and Familiar.Player:HasTrinket(CustomTrinkets.TORN_PAGE) then
...
function rplus:KnifeUpdate(Knife)
	if Knife.FrameCount == 1 and (Knife.Parent or Knife.SpawnerEntity) then
...
function rplus:PickupAwardSpawn(_, Pos)
	...
	for i = 0, game:GetNumPlayers() - 1 do
		...
		if player:HasCollectible(CustomCollectibles.RED_KING) and room:GetType() == RoomType.ROOM_BOSS and level:GetStage() < 9 then
